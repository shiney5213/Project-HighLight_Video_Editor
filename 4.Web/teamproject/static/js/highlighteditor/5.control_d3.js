...
$(function() {
	...
	function searchGame(){
		...
		.done(function(data){
				...
				// 두번째 버전 v4(https://codelib.tistory.com/6)
				// var searchSvg = d3.select("#searchSvg");
				// var width  = parseInt(searchSvg.style("width"), 10);
				// var height = parseInt(searchSvg.style("height"), 10)-20;
				// var xScale = d3.scaleBand()
				// .domain(dataset.map(function(d) { return d.x;} ))
				// .range([0, width]).padding(0.2);

				// searchSvg.selectAll("rect")
				// .data(dataset)
				// .enter()
				// .append("rect")
				// .attr("class","bar")
				// .attr("height", function(d, i) {return (d.y * 50)})
				// .attr("width", xScale.bandwidth())
				// .attr("x", function(d, i) {return xScale(d.x)})
				// .attr("y", function(d, i) {return (d.y*50)});

				// searchSvg.selectAll("text")
				// .data(dataset)
				// .enter()
				// .append("text")
				...
				//세번째 버전: v5
				// var w =parseInt($searchGraph.style("width"), 10);
				// var h = parseInt($searchGraph.style("height"), 10)-20;
				// var w = 900;
				// var h = 100;
				// var padding = 20;
								
				// var svg = d3.select("#searchGraph").append('svg')
				// 	.attr("width", w)
				// 	.attr("height", h);
				...
				// .enter()         
				// .append('rect')
				// .attr('height', function(d,i) {return d.y* 50})  //도형 크기
				// .attr('width', $hancan2)
				// .attr('x', function(d,i){return (($hancan2) * i)})   //도형 위치
				// .attr('y', function(d,i){return (100-d.y*50)}) //도형 위치
				// .attr('background-color', '#cccccc')
				// .attr('class', 'search');
	...
	$('.analysisBtn').on('click', function(){
		...
		$('.hide_until_analysis').css('visibility', 'hidden');
